    ...
    def get_active_filter(self, in_channel, kernel_size):
        ...
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init,
                (self.max_in_channels, self.max_in_channels),
...
class DynamicConv2d(nn.Module):
    ...
    def get_active_filter(self, out_channel, in_channel):
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init,
                (self.max_in_channels, self.max_out_channels),
                (in_channel, out_channel),
            )
        else:
            scaling_factor = 1.0

        return scaling_factor * self.conv.weight[:out_channel, :in_channel, :, :]
...
class DynamicGroupConv2d(nn.Module):
    ...
    def forward(self, x, kernel_size=None, groups=None):
        ...
        y = F.conv2d(x, filters, None, self.stride, padding, self.dilation, groups,)
        return y
...
class DynamicBatchNorm2d(nn.Module):
    ...
    def __init__(self, max_feature_dim):
        ...
        self.bn = nn.BatchNorm2d(self.max_feature_dim, momentum=1)
...
class DynamicSE(SEModule):
    def __init__(self, max_channel, model_init="he_fout", scaling=False):
        super(DynamicSE, self).__init__(max_channel)
        self.model_init = model_init
        self.max_mid_channel = make_divisible(
            max_channel // self.reduction, divisor=MyNetwork.CHANNEL_DIVISIBLE
        )
        self.max_in_channel = max_channel
        self.scaling = scaling
    ...
    def get_active_reduce_weight(self, num_mid, in_channel, groups=None):
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init,
                (self.max_in_channel, self.max_mid_channel),
                (in_channel, num_mid),
            )
        else:
            scaling_factor = 1.0
        ...
        if groups is None or groups == 1:
            return scaling_factor * self.fc.reduce.weight[:num_mid, :in_channel, :, :]
        else:
            ...
            return scaling_factor * torch.cat(
    ...
    def get_active_reduce_bias(self, num_mid):
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init, (1, self.max_mid_channel), (1, num_mid)
            )
        else:
            scaling_factor = 1
        return (
            scaling_factor * self.fc.reduce.bias[:num_mid]
            if self.fc.reduce.bias is not None
            else None
        )
    ...
    def get_active_expand_weight(self, num_mid, in_channel, groups=None):
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init,
                (self.max_mid_channel, self.max_in_channel),
                (num_mid, in_channel),
            )
        else:
            scaling_factor = 1.0
        ...
        if groups is None or groups == 1:
            return scaling_factor * self.fc.expand.weight[:in_channel, :num_mid, :, :]
        else:
            assert in_channel % groups == 0
            sub_in_channels = in_channel // groups
            sub_filters = torch.chunk(
    ...
    def get_active_expand_bias(self, in_channel, groups=None):
        if self.scaling:
            scaling_factor = get_scaling_factor(
                self.model_init, (1, self.max_in_channel), (1, in_channel)
            )
        else:
            scaling_factor = 1.0
        ...
        if groups is None or groups == 1:
            return (
                scaling_factor * self.fc.expand.bias[:in_channel]
    ...
    def forward(self, x, groups=None):
        ...
        if self.scaling:
            scaling_factor_reduce = get_scaling_factor(
                self.model_init,
                (self.max_in_channel, self.max_mid_channel),
                (in_channel, num_mid),
            )
            scaling_factor_expand = get_scaling_factor(
                self.model_init,
                (self.max_mid_channel, self.max_in_channel),
                (num_mid, in_channel),
            )
        else:
            scaling_factor_reduce = 1.0
            scaling_factor_expand = 1.0

        # reduce
        reduce_filter = (
            scaling_factor_reduce
            * self.get_active_reduce_weight(
                num_mid, in_channel, groups=groups
            ).contiguous()
        )
        reduce_bias = scaling_factor_reduce * self.get_active_reduce_bias(num_mid)
        ...
        # expand
        expand_filter = (
            scaling_factor_expand
            * self.get_active_expand_weight(
                num_mid, in_channel, groups=groups
            ).contiguous()
        )
        expand_bias = scaling_factor_expand * self.get_active_expand_bias(
